...
function Input.open()
	--if not getElementData(localPlayer, "queenPlayer.ACTIVE_UI") == 'scoreboard' then return end
	local ACTIVE_UI = getElementData(localPlayer, "queenPlayer.ACTIVE_UI")

	if ACTIVE_UI and ACTIVE_UI ~= 'chat' then
		return
	end
	...
	localPlayer:setData("queenPlayer.ACTIVE_UI", "chat")
	return true
end
...
function Input.close()
	...
	localPlayer:setData("queenPlayer.ACTIVE_UI", false)
	return true
end
...
local function send()
	...
	if inputText ~= "" then
		...
		if utf8.sub(inputText, 1, 1) == "/" then
			...
			triggerEvent("queenChat.message", root, inputText)
		end
